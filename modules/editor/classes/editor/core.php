...
abstract class Editor_Core {
	...
	public function render($display = FALSE, $name = NULL, $create_new = TRUE)
	{
		if ($name)
		{
		...
		$template = View::factory($this->template)
			...
			->set('name', (isset($this->_options['name'])?$this->_options['name']:'teste'))
			->set('options', $this->_options);

		$result = '';
		if ($create_new)
		{
			$result .= "<textarea name='".(isset($this->_options['name'])?$this->_options['name']:'teste')."' id='".(isset($this->_options['name'])?$this->_options['name']:'teste')."'>".htmlspecialchars($this->_options['value'])."</textarea>";
		}
